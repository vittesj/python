...
def json_writer(in_dct: dict, path: str, file_name: str) -> None:
...
def pickle_writer(in_dct: dict, path: str, file_name: str) -> None:
    file_path = os.path.join(path, file_name + '.bin')
    with open(file_path, 'wb') as f_out:
        pickle.dump(in_dct, f_out)


def dct_formatter(total_dct: dict[str, dict[str]], path: str, item_name: str, item_type: str) -> None:
    if item_type == 'F':
...
def dir_walker(aim_path: str, total_dct: dict = None) -> dict[str, dict[str]]:
    if total_dct is None:
        ...
        dct_formatter(total_dct, os.path.join(*basic_path[:-1]), basic_path[-1], 'D')
...
def main():
    tst_path = '/Users/user/dive into python/'
    result = dir_walker(tst_path)
    json_writer(result, os.getcwd(), 'result')
    pickle_writer(result, os.getcwd(), 'result')
